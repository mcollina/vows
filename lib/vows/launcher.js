...
launcher.prototype.loadSuite = function(filename, callback) {
    fs.readFile(filename, function(err, data) {
        if (err) throw err;

        var context = { 
            // todo add all global variables provided by nodejs
            'require' : require,
            '__filename' : filename,
...
launcher.prototype.importSuites = function (files, callback) {
    ...
    files.forEach(function (file) {
        that.loadSuite(file, function(obj) {
            suites = suites.concat(Object.keys(obj).map(function (s) {
                return obj[s];
            }));
            counter += 1; 
...
launcher.prototype.run = function(args) {
    ...
    if (! this.options.watch) {
        ...
        this.importSuites(files, function(suites) {
            that.runSuites(suites, function (results) {
                !options.verbose && _reporter.print('\n');
                l.msg('runner', 'finish');
        ...
        (function () {
            ...
            function esc(str)        { print("\x1b[" + str) }
                ...
                function changed(file) {
                    ...
                    running ++;
                    l.importSuites(files, function (suites) {
                        l.runSuites(suites, function (results) {
                            delete(results.time);
